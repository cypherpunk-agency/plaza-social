...
/**
 * Where the recipient lookup has got to.
 *
 * ⭐ `unmapped` AND `unavailable` ARE DIFFERENT SCREENS, and merging them is the bug this modal was
 * reported for. The seam used to answer `string | null`, so "the chain has no mapping for them" and
 * "we could not reach the chain" arrived identically, and this modal turned both into
 * *"they have never made a transaction from their Polkadot account"* — a confident claim about a
 * stranger, generated by our own failure. See `RecipientResolution` in `lib/host/types.ts`.
 */
type Recipient =
  | { state: 'looking' }
...
export function TipModal({
  ...
  /** Bumped by TRY AGAIN. A transport failure is worth one more attempt before giving up. */
  ...
  useEffect(() => {
    ...
    void payments.resolveRecipient(recipientAddress).then((resolution) => {
      if (!live) return;
      setRecipient(
        resolution.status === 'ready'
          ? { state: 'ready', destination: resolution.destination }
  ...
  }, [isOpen, payments, recipientAddress, retry]);
      ...
      <div className="relative z-10 w-full max-w-sm mx-4 border-2 border-yellow-500 bg-black">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/*
            ...
            `payment.request` needs a 32-byte account and Plaza only ever holds a 20-byte H160. The
            ...
            ⚠️ AND WE ONLY SAY THIS WHEN THE CHAIN ACTUALLY ANSWERED. The `unavailable` branch below
            is the other half of the same protection: we still refuse, but we do not invent a
            biography for the recipient out of our own inability to read a storage map.
          */}
          {payments && recipient.state === 'unmapped' && (
          ...
          {/* ⛔ OUR FAILURE, SAID AS OURS. Same refusal, opposite subject. */}
          {payments && recipient.state === 'unavailable' && (
            <div className="p-3 border border-yellow-700 bg-yellow-950/20 space-y-2">
              <p className="text-yellow-500 font-mono text-xs font-bold">COULDN'T CHECK</p>
              <p className="text-primary-300 font-mono text-xs leading-relaxed">
                Plaza could not look up which Polkadot account {who}'s address belongs to, so it does
                not know where the {CASH_SYMBOL} would go. This is a problem at our end, not
                something {who} has or hasn't done.
              </p>
              <p className="text-primary-500 font-mono text-xs leading-relaxed break-words">
                {recipient.reason}
              </p>
              <button
                type="button"
                onClick={() => setRetry((n) => n + 1)}
                className="px-3 py-1 border border-yellow-700 text-yellow-500 font-mono text-xs hover:border-yellow-500 hover:bg-yellow-950 transition-colors"
              >
                TRY AGAIN
              </button>
            </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {recipient.state === 'ready' || recipient.state === 'looking' ? 'CANCEL' : 'CLOSE'}
            </button>
            {(recipient.state === 'ready' || recipient.state === 'looking') && (
              <button
                type="submit"
                disabled={!canSend}
